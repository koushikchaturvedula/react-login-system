...
async function getAccountByEmail(email) {
  try {
    const result = await pool.query(
      'select * from account a inner join employee_user b on a.userid = b.userid where b.email=$1',
      [email]
    );
...
Router.post('/account', authMiddleware, async (req, res) => {
  const { account_no, bank_name, ifsc, accnt_type  } = req.body;
  try {
    await pool.query(
      'insert into account(account_no,bank_name,ifsc,userid,accnt_type) values($1,$2,$3,$4,$5)',
      [account_no, bank_name, ifsc, req.user.userid,accnt_type]
    );
    res.status(201).send();
  } catch (error) {
    res.send({
